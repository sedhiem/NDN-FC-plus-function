...
public:
  ...
  run()
  {
    ...
    m_face->setInterestFilter(m_funcName, bind(&Func::onInterest, this, _1, _2), ndn::RegisterPrefixSuccessCallback(),
...
private:
  ...
  onInterest(const ndn::InterestFilter& filter, const ndn::Interest& interest)
  {
    ...
    if (interest.getName().get(-1).toSegment() == 0) {
      ofstream outputfile("InterestArray.txt");
      outputfile << interest.getFunction();
      outputfile << ",";
      outputfile << interest.getName();
      outputfile.close();
    }
    ...
    // store Dostore;
    /*for(int i=0; i<20; i++){
      if(Dostore.InterestArray[0][i] == interest.getName() &&
      Dostore.InterestArray[1][i] == interest.getFunction()){
        break;
      }
      if(Dostore.InterestArray[0][i] == NULL){
        Dostore.InterestArray[0][i] = interest.getName();
        break;
      }
      }*/
    // Dostore.Test();
    // InterestArray[0][0] = interest.getName();
    if (segment > 0) {
      ...
      // interest.setEraseCache(0);
      m_face->expressInterest(interest, bind(&Func::onData, this, _1, _2), bind(&Func::onNack, this, _1, _2),
...
func(int num, char** input)
{
  ...
  try {
    Func function;
    function.setFuncName(ndn::Name(input[1]));
    function.run();
  }
  catch (const std::exception& e) {
    std::cerr << "ERROR: " << e.what() << std::endl;
...
main(int argc, char** argv)
{
  std::thread function(func, argc, argv);
  function.join();
